/*
 * RxFrame.h
 * copied from RxFrame.h:https://github.com/gehee/FPVue_android.git
 * thanks to geehe
 */
//
...
public:
  ...
  bool MatchesChannelID(const uint8_t *channel_id) const {
    //        0x57, 0x42, 0xaa, 0xbb, 0xcc, 0xdd,   // last four bytes are
    //        replaced by channel_id (x2)
    return _data[10] == 0x57 && _data[11] == 0x42 &&
           _data[12] == channel_id[0] && _data[13] == channel_id[1] &&
           _data[14] == channel_id[2] && _data[15] == channel_id[3] &&
           _data[16] == 0x57 && _data[17] == 0x42 &&
           _data[18] == channel_id[0] && _data[19] == channel_id[1] &&
           _data[20] == channel_id[2] && _data[21] == channel_id[3];
  }

private:
  bool IsDataFrame() const {
    return _data.size() >= 2 && _data[0] == _dataHeader[0] &&
